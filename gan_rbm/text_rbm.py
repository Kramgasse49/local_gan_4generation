...
class TRBM(object):
    ...
    def build(self):
        """
        """
        self.W_rr, self.W_rq = ( Variable(torch.randn(self.r_size, self.h_size) * 0.01),
                                 Variable(torch.randn(self.q_size, self.h_size) * 0.01))
        self.b_q, self.b_r, self.c_r = ( Variable(torch.ones(self.q_size) * 0.001),
                                         Variable(torch.ones(self.r_size) * 0.001),
                                         Variable(torch.ones(self.h_size) * 0.001)) 
    ...
    def hr_free_energy(self, q_pos, r_pos):
        ...
        hidden_term = torch.sum( F.logsigmoid(-wx_b), dim=1)
        # check inf
